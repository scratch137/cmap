...
int vane_obscal(char *inp)
{
	...
	if (!strcasecmp(inp, "obs") || !strcasecmp(inp, "0")) {
		I2CStatus = writeBEX(~VANEOUTPORT, SBBEX_ADDR);  // pin value low to drive to obs, all others high
    	if (!I2CStatus) {
		...
		I2CStatus = writeBEX(~VANEINPORT, SBBEX_ADDR);  // pin value low to drive to cal, all others high
    	if (!I2CStatus) {
...
void init_vane(void)
{
	...
	I2CStat = I2CSEND1;
	I2CStat = configBEX(0xff, SBBEX_ADDR);  // configure BEX for write on all control ports
	vanePar.vaneFlag = (I2CStat ? I2CStat : 1);
	writeBEX(0xff, SBBEX_ADDR);   // set all ports high; turns off LED on 0x80 (if on) and motor
	switch (vanePar.vaneFlag) {  // record state for output
		case 0 :
			vanePar.vanePos = "obs";
			break;
		case 1 :
	...
	buffer[0] = VANE_SWADDR;
	I2CStat = I2CSEND1;
	writeBEX((BYTE) ~0x80, SBBEX_ADDR);   // turn on LED, set other port values high
	address = SB_SSBADDR;  // Close subsubbus
	buffer[0] = 0x00;
	I2CStat = I2CSEND1;
	address = SB_SBADDR;  // Close subbus
